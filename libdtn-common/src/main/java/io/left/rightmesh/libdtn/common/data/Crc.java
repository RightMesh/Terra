...
/**
 * Crc class provides two crc algorithms, namely crc-16 and crc-32.
 *
...
public abstract class Crc {
     ...
     */
    static final int[] CRC16_X25_TABLE = {
    ...
    /**
     * consumes a buffer for CRC. It will read all the remaining bytes from the ByteBuffer.
     *
     * @param buffer input
     */
    ...
    /**
     * terminate the CRC. After this call, no buffer can be consumed.
     *
     * @return the resulting crc buffer
     */
    public abstract ByteBuffer done();

    /**
     * terminate the CRC and check that the resulting buffer matches the argument.
     * After this call, no buffer can be consumed.
     *
     * @param check expected buffer
     * @return true if the resulting buffer matches the expected buffer, false otherwise.
     */
    ...
    /**
     * Computes a Crc16 over an entire Flowable. This method will consumes every
     * remaining bytes from every bytebuffer observed from this flowable.
     *
     * @param stream of buffer to consume
     * @return the crc as a 2 bytes array
     */
    ...
    /**
     * Computes a Crc32 over an entire Flowable. This method will consumes every
     * remaining bytes from every bytebuffer observed from this flowable.
     *
     * @param stream of buffer to consume
     * @return the crc as a 4 bytes array
     */
