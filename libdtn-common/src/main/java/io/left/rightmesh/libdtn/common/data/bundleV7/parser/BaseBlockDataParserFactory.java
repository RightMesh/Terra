    ...
    public CborParser create(int type, CanonicalBlock block, BLOBFactory blobFactory, Log logger) throws UnknownBlockTypeException {
        switch (type) {
            ...
            case PreviousNodeBlock.type:
                return PreviousNodeBlockParser.getParser((PreviousNodeBlock) block, logger);
            case AgeBlock.type:
            ...
            case BlockAuthenticationBlock.type:
                return SecurityBlockParser.getParser((BlockAuthenticationBlock) block, logger);
            case BlockIntegrityBlock.type:
                return SecurityBlockParser.getParser((BlockIntegrityBlock) block, logger);
            case BlockConfidentialityBlock.type:
                return SecurityBlockParser.getParser((BlockConfidentialityBlock) block, logger);
            default:
