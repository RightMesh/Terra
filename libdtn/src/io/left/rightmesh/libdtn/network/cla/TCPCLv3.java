...
import io.left.rightmesh.libdtn.bundleV6.AsyncSerializer;
import io.left.rightmesh.libdtn.bundleV6.SDNV;
...
import io.left.rightmesh.libdtn.utils.rxdeserializer.RxDeserializer;
...
import io.left.rightmesh.libdtn.utils.rxdeserializer.ShortState;
...
public class TCPCLv3 implements ConvergenceLayer {
    ...
    public static class Channel implements DTNChannel {
        ...
        RxTCP.Connection c;
        DeserializerEmitter<ByteBuffer> recvData;
        ContactHeader ch;
        ...
        public Observable<Bundle> recvBundle() {
            ...
            return Observable.create(observer ->
                    RxDeserializer.<ByteBuffer>create(c.recv(), d -> {
        ...
        private class TCPCLDataReceiver extends DeserializerEmitter<ByteBuffer> {
            // protocol mutables
            ...
            private RxState contact_header_magic = new BufferState(magic) {
                ...
                public void onSuccess(ByteBuffer buffer) throws RxDeserializerException {
                    ...
                    if (!new String(magic).equals("dtn!")) {
                        throw new RxDeserializerException(new String(magic)
            ...
            private RxState contact_header_version = new RxState() {
                ...
                public void onNext(ByteBuffer next) throws RxDeserializerException {
                    ...
                    if (version != 3) {
                        throw new RxDeserializerException("bad TCPCLv3 version ("
                                + version + " != 3)");
                    }
            ...
            private RxState contact_header_keepalive_interval = new ShortState() {
                @Override
                public void onSuccess(Short s) throws RxDeserializerException {
                    interval = s;
            ...
            private BufferState contact_header_local_eid = new BufferState() {
                ...
                public void onSuccess(ByteBuffer buffer) throws RxDeserializerException {
                    try {
                        ...
                        throw new RxDeserializerException(e.getMessage());
                    }
                    onRecvContactHeader(
